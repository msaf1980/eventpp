...
namespace httpc
{
    Conn::Conn(ConnPool * p, eventpp::EventLoop * l)
        : loop_(l)
        , pool_(p)
        , host_(p->host())
        , port_(p->port())
    ...
    Conn::Conn(eventpp::EventLoop * l, const std::string & h, int p, eventpp::Duration t, bool enable_ssl)
        : loop_(l)
        , pool_(nullptr)
        , host_(h)
        , port_(p)
    ...
    bool Conn::Init()
    {
        ...
        if (enable_ssl())
        {
            ...
            bufferevent_ = bufferevent_openssl_socket_new(
                loop_->event_base(), -1, ssl_, BUFFEREVENT_SSL_CONNECTING, BEV_OPT_CLOSE_ON_FREE | BEV_OPT_DEFER_CALLBACKS);
        }
        else
        {
