...
namespace httpc
{
        ...
        : pool_(nullptr), loop_(loop), body_(body)
    {
        ...
        bool is_ssl = scheme && strcasecmp(scheme, "https") == 0;
...
#    endif
    ...
    Request::~Request() { assert(loop_->IsInLoopThread()); }
    ...
    void Request::AddHeader(const std::string & header, const std::string & value) { headers_[header] = value; }
    ...
    void Request::HandleResponse(struct evhttp_request * r)
    {
        ...
        if (!r)
        {
            ...
            {
                LOG_ERROR << "socket error(" << errcode << "): " << evutil_socket_error_to_string(errcode);
            }
        }
#endif
