...
DNSResolver::DNSResolver(EventLoop * evloop, const std::string & h, Duration timeout, const Functor & f)
    : loop_(evloop), dnsbase_(nullptr), dns_req_(nullptr), host_(h), err_(0), timeout_(timeout), functor_(f)
{
...
void DNSResolver::SyncDNSResolve()
{
    ...
    err_ = getaddrinfo(host_.c_str(), nullptr, &hints, &answer);
    if (err_ == 0)
    {
