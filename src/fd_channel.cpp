...
FdChannel::FdChannel(EventLoop * l, eventpp_socket_t f, bool r, bool w) : loop_(l), attached_(false), event_(nullptr), fd_(f)
{
    // DLOG_TRACE << "fd=" << fd_;
...
void FdChannel::HandleEvent(eventpp_socket_t sockfd, short which, void * v)
{
    FdChannel * c = (FdChannel *)v;
    c->HandleEvent(sockfd, which);
}

void FdChannel::HandleEvent(eventpp_socket_t sockfd, short which)
{
    assert(sockfd == fd_);
