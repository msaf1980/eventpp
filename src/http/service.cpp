...
{
#undef H_ARRAYSIZE
#define H_ARRAYSIZE(a) ((sizeof(a) / sizeof(*(a))) / static_cast<size_t>(!(sizeof(a) % sizeof(*(a)))))
    ...
    Service::Service(EventLoop * l, bool enable_ssl, const char * certificate_chain_file, const char * private_key_file)
        : evhttp_(nullptr)
    ...
    {
#else
    Service::Service(EventLoop * l) : evhttp_(nullptr), evhttp_bound_socket_(nullptr), listen_loop_(l)
    {
#endif
    ...
    bool Service::initSSL(bool force_enable)
    {
        ...
        SSL_CTX_set_options(
            ctx, SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE | SSL_OP_NO_SSLv2 /*禁用SSLv2*/ | SSL_OP_NO_TLSv1 /*禁用TLSv1*/);
        ...
        if (1 != SSL_CTX_use_certificate_chain_file(ctx, certificate_chain_file_.c_str()))
        {
            LOG_ERROR << "Load certificate chain file(" << certificate_chain_file_.c_str() << ")failed";
            ERR_print_errors_fp(stderr);
            return false;
        }
        /* 加载私钥文件 */
        if (1 != SSL_CTX_use_PrivateKey_file(ctx, private_key_file_.c_str(), SSL_FILETYPE_PEM))
        {
            LOG_ERROR << "Load private key file(" << private_key_file_.c_str() << ")failed";
            ERR_print_errors_fp(stderr);
            return false;
        }
        /* 校验私钥与证书是否匹配 */
        ...
        auto bevcb = [](struct event_base * base, void * arg) -> struct bufferevent *
        {
            ...
            r = bufferevent_openssl_socket_new(base, -1, SSL_new(sslctx), BUFFEREVENT_SSL_ACCEPTING, BEV_OPT_CLOSE_ON_FREE);
            return r;
        };
    ...
    void Service::RegisterHandler(const std::string & uri, HTTPRequestCallback callback) { callbacks_[uri] = callback; }

    void Service::RegisterDefaultHandler(HTTPRequestCallback callback) { default_callback_ = callback; }
    ...
    struct Response
    {
        Response(const ContextPtr & c, const std::string & m) : ctx(c)
        {
            if (m.size() > 0)
            {
                buffer = evbuffer_new();
    ...
    void Service::SendReply(const ContextPtr & ctx, const std::string & response_data)
    {
        ...
        auto f = [this, response]() {
            ...
            if (!response->buffer)
            {
                evhttp_send_reply(x->req(), HTTP_NOTFOUND, g_http_code_string[HTTP_NOTFOUND], nullptr);
                return;
            }

            assert(x->response_http_code() <= kMaxHTTPCode);
            assert(x->response_http_code() >= 100);
            evhttp_send_reply(x->req(), x->response_http_code(), g_http_code_string[x->response_http_code()], response->buffer);
        };
