...
{
TCPConn::TCPConn(
    EventLoop * l, const std::string & n, evpp_socket_t sockfd, const std::string & laddr, const std::string & raddr, uint64_t conn_id)
    : loop_(l), fd_(sockfd), id_(conn_id), name_(n), local_addr_(laddr), remote_addr_(raddr), type_(kIncoming), status_(kDisconnected)
{
    if (sockfd >= 0)
    {
...
TCPConn::~TCPConn()
{
    // DLOG_TRACE << "name=" << name() << " channel=" << chan_.get() << " fd=" << fd_ << " type=" << int(type())
...
void TCPConn::HandleRead()
{
    ...
    else if (n == 0)
    {
        ...
        else
        {
            ...
            {
                // DLOG_TRACE << "channel (fd=" << chan_->fd() << ") DisableReadEvent. delay time " << close_delay_.Seconds()
                //            << "s. We close this connection immediately";
                DelayClose();
            }
            else
            {
                ...
                //            << ") DisableReadEvent. And set a timer to delay close this TCPConn, delay time " << close_delay_.Seconds()
                //            << "s";
                delay_close_timer_ = loop_->RunAfter(
                    close_delay_, std::bind(&TCPConn::DelayClose, shared_from_this())); // TODO leave it to user layer close.
            }
        }
    }
    else
