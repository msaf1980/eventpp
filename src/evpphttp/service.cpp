...
#include <eventpp/evpphttp/service.hpp>
...
int Service::RequestHandler(const eventpp::TCPConnPtr& conn, eventpp::Buffer* buf, HttpRequest& hr) {
...
void Service::OnMessage(const eventpp::TCPConnPtr& conn, eventpp::Buffer* buf) {
    int ret = 0;
    //LOG_TRACE << "recv message:" << buf->ToString();
    // if (!conn->context().IsEmpty()) {
    if (!conn->context().has_value()) {
        auto context = conn->context();
        HttpRequest *hr = std::any_cast<HttpRequest *>(context);
    ...
    while (buf->size() > 0) {
        ...
        if (ret > 0) {
            std::any context(new HttpRequest(std::move(hr)));
