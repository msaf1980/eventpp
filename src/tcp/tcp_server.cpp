...
bool TCPServer::Start()
{
    ...
    if (rc)
    {
        ...
        listener_->SetNewConnectionCallback(
            std::bind(&TCPServer::HandleNewConn, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void TCPServer::StopInLoop(DoneCallback on_stopped_cb)
{
    ...
    else
    {
        for (auto & c : connections_)
        {
            ...
            {
                // DLOG_TRACE << "Do not need to call Close for this TCPConn it may be doing disconnecting. TCPConn=" << c.second.get()
                //            << " fd=" << c.second->fd() << " status=" << StatusToString();
            }
        }
...
void TCPServer::HandleNewConn(evpp_socket_t sockfd, const std::string & remote_addr /*ip:port*/, const struct sockaddr_in * raddr)
{
