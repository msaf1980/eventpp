...
{
public:
    Session(eventpp::EventLoop * loop, const std::string & serverAddr /*ip:port*/, const std::string & name, Client * owner)
        : client_(loop, serverAddr, name), owner_(owner), bytes_read_(0), bytes_written_(0), messages_read_(0)
    {
        ...
        client_.set_connecting_timeout(eventpp::Duration(10.0));
    }
...
private:
    void OnConnection(const eventpp::TCPConnPtr & conn);
...
public:
    Client(
        ...
        int timeout_sec,
        int threadCount)
        : loop_(loop), name_(name), session_count_(sessionCount), timeout_(timeout_sec), connected_count_(0)
    {
        loop->RunAfter(eventpp::Duration(double(timeout_sec)), std::bind(&Client::HandleTimeout, this));
        tpool_.reset(new eventpp::EventLoopThreadPool(loop, threadCount));
        tpool_->Start(true);
    ...
    void OnDisconnect(const eventpp::TCPConnPtr & conn)
    {
        if (--connected_count_ == 0)
        {
            ...
            LOG_WARN << "name=" << name_ << " " << static_cast<double>(totalBytesRead) / static_cast<double>(totalMessagesRead)
...
private:
    ...
    std::string name_;
    std::shared_ptr<eventpp::EventLoopThreadPool> tpool_;
    int session_count_;
    int timeout_;
...
void Session::OnConnection(const eventpp::TCPConnPtr & conn)
{
    if (conn->IsConnected())
    {
