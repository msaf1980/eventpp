...
public:
    Session(
        eventpp::EventLoop * loop, const std::string & serverAddr /*ip:port*/, const std::string & name, size_t block_size, Client * owner)
        : client_(loop, serverAddr, name), owner_(owner), bytes_read_(0), bytes_written_(0), messages_read_(0), block_size_(block_size)
    {
    ...
    void OnDisconnect(const eventpp::TCPConnPtr & conn)
    {
        if (--connected_count_ == 0)
        {
            ...
            printf(
                ...
                session_count_,
                total_errors_,
                (double)totalBytesRead / totalMessagesRead,
                (double)totalBytesRead / (timeout_ * 1024 * 1024),
                totalMessagesRead / timeout_,
                (double)100000 * timeout_ / totalMessagesRead,
                totalBytesRead,
                totalMessagesRead);
