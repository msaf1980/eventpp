#ifndef __EVENTPP_HTTP_RESPONSE_HPP__
#define __EVENTPP_HTTP_RESPONSE_HPP__
...
#include <eventpp/evpphttp/http_request.hpp>
...
#define is_connection_xx(name, key) \
    ...
    public:
        ...
        void SendReply(
            const eventpp::TCPConnPtr & conn,
            const int response_code,
            const std::map<std::string, std::string> & header_field_value,
            const std::string & response_body);
        void MakeHttpResponse(
            const int response_code, const int64_t body_size, const std::map<std::string, std::string> & header_field_value, Buffer & buf);
    ...
    private:
        ...
        void SendContinue(const eventpp::TCPConnPtr & conn);
...
#endif // __EVENTPP_HTTP_RESPONSE_HPP__
