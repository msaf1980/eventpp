#ifndef __EVENTPP_TCP_SERVER_HPP__
#define __EVENTPP_TCP_SERVER_HPP__
...
// </code>
//
class EVENTPP_EXPORT TCPServer : public ThreadDispatchPolicy, public ServerStatus
{
public:
    ...
    TCPServer(EventLoop * loop, const std::string & listen_addr /*ip:port*/, const std::string & name, uint32_t thread_num);
    ~TCPServer();
...
private:
    ...
    void HandleNewConn(eventpp_socket_t sockfd, const std::string & remote_addr /*ip:port*/, const struct sockaddr_in * raddr);
...
#endif // __EVENTPP_TCP_SERVER_HPP__