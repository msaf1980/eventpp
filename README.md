...
<a href="https://github.com/masf1980/eventpp/releases">Releases</a>
[![Project Status: WIP – The project not a stable.](http://www.repostatus.org/badges/latest/wip.svg)](http://www.repostatus.org/#wip)


# Introduction

[eventpp] is a C++ network library for developing high performance network services using TCP/UDP/HTTP protocols. For I/O reactor library libevent2 is used.

Originally based on https://github.com/Qihoo360/evpp

[eventpp] provides a TCP Server to support multi-threaded nonblocking event-driven server and also a HTTP, UDP Server to support HTTP and UDP protocols.
...
1. A nonblocking multi-threaded TCP server (TODO)
1. A nonblocking TCP client (TODO)
1. A nonblocking multi-threaded HTTP server based on the buildin http server of libevent (TODO)
1. A nonblocking HTTP client (TODO)
1. A nonblocking multi-threaded UDP server (TODO)
1. Async DNS resolving (TODO)
1. EventLoop/ThreadPool/Timer
2. Easy install — [eventpp] can be packaged as a deb, rpm, tar.gz with a single command for straight forward distribution and integration

NOTE: master is development branch and may not be stable at all times.

# Origin


Requirements to ideal C++ network library must have the features below: 

1. A simple enough C++ interface
2. Support multi-threads and multi-processes
3. Based on [libevent] is preferable for my projects. Given my older applications were based on [libevent], it was preferable to have your new framework also be based on it, so as to reduce the overall time/effort/cost to completion.

So we developed one ourself. The design of the interface is highly inspired by [muduo] and [Golang]. Let's take some examples to explain this: 
